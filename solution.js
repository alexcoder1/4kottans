...
var createPoly = function(points) {
    ...
    for(var i=0 ; i < points.length ; i++) {    
        res.addPoint(new Point(points[i].x,points[i].y));
    }
    return res;
}
...
//работает с несамопересекающимися полигонами,
//Поскольку intersects разобьет самопересечение на разные контуры, это и не нужно:)
function square(poligon){
var s,res=0;
var n=poligon.length; 
   // Расчет площади многоугольника через сумму площадей трапеций 
   for (i = 0; i < n; i++) {
     if (i == 0) {
       s = poligon[i][0]*(poligon[n-1][1] - poligon[i+1][1]); //если i == 0, то y[i-1] заменяем на y[n-1]
     }
     else
       if (i == n-1) {
	 s = poligon[i][0]*(poligon[i-1][1] - poligon[0][1]); // если i == n-1, то y[i+1] заменяем на y[0]
       }
     else {
       s = poligon[i][0]*(poligon[i-1][1] - poligon[i+1][1]);
     }
       res += s;
   }
...
var poly1 = createPoly(fig1);
var poly2 = createPoly(fig2);
var result=[];
...
for(i=0;i<num;i++) {
	...
	var s=square(vertices);
	if(s>=0.0001){
		result[k]=[];
		for(j=0;j<vertices.length;j++){
			result[k][j]=[];
			result[k][j].x = vertices[j][0];
			result[k][j].y = vertices[j][1];
			}
		k++;
		}
	}
